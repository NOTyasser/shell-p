#include "shell.h"

/**
 * Interactive-Returns true if the shell is in interactive mode.
 * @info:The structure address.
 * Returns:1 if in interactive mode, 0 if not.
 */

int interactive(info_t *info)
{
return (isatty(STDIN_FILENO) && info->readfd <= 2);
}

/**
 * is_delim-It checks if the char is a delimeter.
 * @c:Is the character that is being checked.
 * @delim:Is the delimeter string.
 * Returns:1 if true, 0 if not.
 */

int is_delim(char c, char *delim)
{
while (*delim)
if (*delim++ == c)
return (1);
return (0);
}

/**
 * _isalpha - Checks for the alphabetic character.
 * @c: The character to input.
 * Returns: 1 if c is an alphabet, 0 if not.
 */

int _isalpha(int c)
{
if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
return (1);
else
return (0);
}

/**
 * _atoi - Converts the stringto an integer.
 * @s -Is the string that will be converted.
 * Returns:0 if there is no string ,converted numbers if there is'
 */

int _atoi(char *s)
{
int i, sign = 1, flag = 0, output;
unsigned int result = 0;

for (i = 0; s[i] != '\0' && flag != 2; i++)
{
if (s[i] == '-')
sign *= -1;

if (s[i] >= '0' && s[i] <= '9')
{
flag = 1;
result *= 10;
result += (s[i] - '0');
}
else if (flag == 1)
flag = 2;
}

if (sign == -1)
output = -result;
else
output = result;

return (output);
}

